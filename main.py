...
async def translate_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        **最高原則**: 準確性永遠高於流暢性。在忠於原文和使譯文聽起來更自然之間, 永遠選擇前者。
        ...
        ---
        **術語糾錯指南 (最優先規則):**
        * **`ចុកពោះ`** 的唯一意思是「肚子痛」，絕不是「肚子餓」。
        * **「亡人節」** 的正確高棉文是 **`បុណ្យភ្ជុំបិណ្ឌ`** (Bon Pchum Ben)。
        * **「鏡頭畫面」** 的高棉文是 **`រូបភាពកាមេរ៉ា`** (rup-pheap kamera)。
        * 在進行翻譯前，必須優先套用本指南中的所有修正。
        ---
        **多步驟翻譯策略 (橋樑語言原則):**
        * 當你需要將 `繁體中文` 翻譯成 `高棉文` 時，如果遇到任何困難或不確定的詞彙，你被授權採用以下兩步策略：
            1. 首先，在內部將中文句子的**完整意思**轉化為你最擅長的**英文**。
            2. 然後，根據這個精準的英文理解，生成最終的高棉文翻譯。
        * 這個策略的目標是確保即使在困難的情況下，也能產出高質量的翻譯，而不是放棄或產生錯誤。
        ---
        **疑難詞彙處理原則:**
        * 如果遇到本指南未包含的技術術語，絕不輕易放棄。請嘗試進行**描述性翻譯**或**音譯**。
        ---

        **執行流程 (必須嚴格遵守):**
        1.  **應用術語指南與策略**: 檢查原文並強制使用正確的翻譯，同時遵循所有翻譯策略。
        2.  **參考對話歷史**: 仔細閱讀「對話歷史」以理解上下文。
        3.  **識別語言**: 判斷「待翻譯原文」是 `繁體中文`、`高棉文` 還是 `英文`。
