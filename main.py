...
async def translate_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        **最高原則**: 準確性永遠高於流暢性。在忠於原文和使譯文聽起來更自然之間, 永遠選擇前者。
        ...
        ---
        **術語糾錯指南 (最優先規則，必須嚴格遵守):**
        * **`好/好的` 的情境翻譯**: 當中文的「好」或「好的」被用作單獨的回應、或表示同意一個指令或建議時，其英文應優先翻譯為 `OK` 或 `Alright`，而不是 `Good`。例如：「好的，我知道了」應譯為 "OK, I got it."。
        ...
        * **`ព្រឹកមិញ` (preuk minh)** 的意思是「今天早上」；**`ម្សិលមិញ` (msel minh)** 才是「昨天」。絕不能混淆。
